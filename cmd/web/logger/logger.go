...
func (ch ContextHandler) Handle(ctx context.Context, r slog.Record) error {
	r.AddAttrs(ch.addTraceFromContext(ctx)...)
...
// addTraceFromContext todo add trace for datadog
func (ch ContextHandler) addTraceFromContext(ctx context.Context) []slog.Attr {
	return []slog.Attr{}
}
...
func getDefaultValueFromContext(ctx context.Context, key string) string {
	value := ""
	if ctxValue, ok := ctx.Value(key).(string); ok {
		value = ctxValue
	}
	return value
}
